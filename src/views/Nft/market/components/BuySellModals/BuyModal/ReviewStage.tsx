    ...
    <>
      <Flex px="24px" pt="24px" flexDirection="column">
        ...
        <BorderedBox>
          ...
          <ButtonMenu
            ...
            variant="subtle"
          >
            <ButtonMenuItem>SNT</ButtonMenuItem>
            <ButtonMenuItem>WSNT</ButtonMenuItem>
          </ButtonMenu>
          <Text small color="textSubtle">
            {t('Total payment')}
          </Text>
          ...
          <Text small color="textSubtle">
            {t('%symbol% in wallet', { symbol: paymentCurrency === PaymentCurrency.BNB ? 'SNT' : 'WSNT' })}
          </Text>
          {!account ? (
          ...
          <Message p="8px" variant="danger">
            <Text>
              {t('Not enough %symbol% to purchase this NFT', {
                symbol: paymentCurrency === PaymentCurrency.BNB ? 'SNT' : 'WSNT',
              })}
            </Text>
          </Message>
        )}
        <Flex alignItems="center">
          <Text my="16px" mr="4px">
            {t('Convert between BNB and WSNT for free')}:
          </Text>
          <Button
            as={Link}
            p="0px"
      ...
      <Flex px="24px" pb="24px" flexDirection="column">
        ...
        <Button as={Link} external style={{ width: '100%' }} href="/swap?outputCurrency=BNB" variant="secondary">
          {t('Get %symbol1% or %symbol2%', { symbol1: 'SNT', symbol2: 'WSNT' })}
        </Button>
      </Flex>
    </>
  )
