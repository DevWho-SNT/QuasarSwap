...
export const generateMetaData = () => {
  return {
    plugins: {},
    network: 87,
    strategies: [{ name: 'cake', params: { symbol: 'CAKE', address: tokens.cake.address, decimals: 18 } }, { name: 'syrup', params: { symbol: 'SYRUP', address: tokens.syrup.address, decimals: 18 } }, { name: 'qlb', params: { symbol: 'QLB', address: tokens.qlb.address, multiplier: 10000 } }],
...
function calculateVotingPower(scoresList: GetScoresResponse, voters: string[], scoresListIndex: ScoresListIndex) {
  ...
  syrupBalances = scoresListIndex.syrupBalances > -1 ? scoresList[scoresListIndex.syrupBalances] : defaultScore
  qlbBalances = scoresListIndex.qlbBalances > -1 ? scoresList[scoresListIndex.qlbBalances] : defaultScore
  ...
  const result = voters.map((address) => {
    ...
    const syrupBalance = new BigNumber(syrupBalances[address] ?? 0)
    const qlbBalance = new BigNumber(qlbBalances[address] ?? 0)
    ...
    return {
      ...
      syrupBalance: syrupBalance.div(TEN_POW_18).toFixed(18),
      qlbBalance: qlbBalance.div(TEN_POW_18).toFixed(18),
...
function verifyDefaultContract(blockNumber: number) {
  return {
    ...
    Syrup: ContractDeployedNumber.Syrup < blockNumber,
    CakeVault: ContractDeployedNumber.CakeVault < blockNumber,
    QLBBalance: ContractDeployedNumber.QLBBalance < blockNumber,
...
export async function getVotingPowerList(voters: string[], poolAddresses: string[], blockNumber: number) {
  ...
  if (contractsValid.Syrup) {
    defaultStrategy.push(snapshotStrategies[1])
    scoresListIndex.syrupBalances = indexCounter++
  }
  if (contractsValid.QLBBalance) {
    defaultStrategy.push(snapshotStrategies[2])
    scoresListIndex.qlbBalances = indexCounter++
  }
  ...
  //   defaultStrategy.push(snapshotStrategies[9])
