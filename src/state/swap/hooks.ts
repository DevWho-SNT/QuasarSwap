...
import { Currency, CurrencyAmount, ETHER, JSBI, Token, TokenAmount, Trade } from '@quasarswap/sdk'
  ...
  const onCurrencySelection = useCallback(
    (field: Field, currency: Currency) => {
      dispatch(
        selectCurrency({
          field,
          currencyId: currency instanceof Token ? currency.address : currency === ETHER ? 'SNT' : '',
        }),
      )
    },
    [dispatch],
...
function parseCurrencyFromURLParameter(urlParam: any): string {
  if (typeof urlParam === 'string') {
    ...
    if (urlParam.toUpperCase() === 'SNT') return 'SNT'
    if (valid === false) return 'SNT'
  }
  return ''
}
