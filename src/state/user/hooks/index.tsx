import { ChainId, Pair, Token } from '@quasarswap/sdk'
...
export function useUserNFTFarmStakedOnly(isActive: boolean): [boolean, (stakedOnly: boolean) => void] {
  const dispatch = useDispatch<AppDispatch>()
  const userNFTFarmStakedOnly = useSelector<AppState, AppState['user']['userNFTFarmStakedOnly']>((state) => {
    return state.user.userNFTFarmStakedOnly
  })

  const setUserNFTFarmStakedOnly = useCallback(
    (stakedOnly: boolean) => {
      const nftFarmStakedOnly = stakedOnly ? NFTFarmStakedOnly.TRUE : NFTFarmStakedOnly.FALSE
      dispatch(updateUserNFTFarmStakedOnly({ userNFTFarmStakedOnly: nftFarmStakedOnly }))
    },
    [dispatch],
  )

  return [
    userNFTFarmStakedOnly === NFTFarmStakedOnly.ON_FINISHED ? !isActive : userNFTFarmStakedOnly === NFTFarmStakedOnly.TRUE,
    setUserNFTFarmStakedOnly,
  ]
}
