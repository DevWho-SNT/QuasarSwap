import { ChainId, Currency, currencyEquals, JSBI, Price } from '@quasarswap/sdk'
...
export default function useBUSDPrice(currency?: Currency): Price | undefined {
  ...
  const tokenPairs: [Currency | undefined, Currency | undefined][] = useMemo(
    () => [
      [chainId && wrapped && currencyEquals(WSNT, wrapped) ? undefined : currency, chainId ? WSNT : undefined],
      [wrapped?.equals(BUSD_MAINNET) ? undefined : wrapped, chainId === ChainId.MAINNET ? BUSD_MAINNET : undefined],
      [chainId ? WSNT : undefined, chainId === ChainId.MAINNET ? BUSD_MAINNET : undefined],
    ],
  ...
  return useMemo(() => {
    ...
    // handle weth/eth
    if (wrapped.equals(WSNT)) {
      if (busdPair) {
        const price = busdPair.priceOf(WSNT)
    ...
    const ethPairETHAmount = ethPair?.reserveOf(WSNT)
    const ethPairETHBUSDValue: JSBI =
      ethPairETHAmount && busdEthPair ? busdEthPair.priceOf(WSNT).quote(ethPairETHAmount).raw : JSBI.BigInt(0)
    ...
    if (ethPairState === PairState.EXISTS && ethPair && busdEthPairState === PairState.EXISTS && busdEthPair) {
      if (busdEthPair.reserveOf(BUSD_MAINNET).greaterThan('0') && ethPair.reserveOf(WSNT).greaterThan('0')) {
        const ethBusdPrice = busdEthPair.priceOf(BUSD_MAINNET)
        const currencyEthPrice = ethPair.priceOf(WSNT)
