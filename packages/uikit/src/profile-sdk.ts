...
class QuasarProfileSdk {
   ...
   * API repo - https://github.com/quasarswap/quasar-profile-api
   */
   ...
   * Contract repo - https://github.com/quasarswap/quasar-contracts/tree/master/projects/profile-nft-gamification
   */
   ...
   * from QuasarProfile contract.
   * NFT's bunnyId is retrieved from QuasarBunnies contract and mapped to static NFT data stored in constant.
   * Contracts repo - https://github.com/quasarswap/quasar-contracts/tree/master/projects/profile-nft-gamification
   */
  getProfile = async (address: string): Promise<GetProfileResponse> => {
    try {
      ...
      if (isActive) {
        ...
        const avatar = nft ? `https://quasarswap.net/images/nfts/${nft.images.sm}` : undefined;
        ...
        Cookies.set(
          ...
          { domain: "quasarswap.net", secure: true, expires: 30 }
        );
      }
...
export default QuasarProfileSdk;
